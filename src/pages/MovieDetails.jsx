...
const [editForm, setEditForm] = useState({ rating: '', comment: '' })
  ...
  if (loading) {
    return <div className="text-white text-center py-20">Loading movie details...</div>
  }
...
const handleDelete = async (id) => {
  ...
  return (
    ...
    <div className="max-w-7xl w-full flex flex-col md:flex-row gap-8 bg-secondary/10 p- rounded-x5">
        ...
        <div className="flex-1 space-y-3">
          <h1 className="text-3xl md:text-5xl font-bold text-accent mb-2">{movie.title}</h1>
...
{editingReviewId === review._id ? (
  <form onSubmit={handleEditSubmit} className="space-y-2">
    ...
    <div className="flex gap-2">
      <button type="submit" className="bg-accent px-3 py-1 rounded text-white text-sm">Save</button>
      <button type="button" onClick={() => setEditingReviewId(null)} className="text-red-500 text-sm">Cancel</button>
    </div>
  </form>
) : (
  <>
    ...
    {review.user?._id === userId && (
      <div className="flex gap-3 mt-2 text-sm">
        <button onClick={() => handleEdit(review)} className="text-yellow-400">Edit</button>
        <button onClick={() => handleDelete(review._id)} className="text-red-400">Delete</button>
      </div>
    )}
  </>
)}
          ...
          {/* Stretch: Watchlist / Trailer Buttons */}
